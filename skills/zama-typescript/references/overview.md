...
- **Relayer API key (mainnet only).** Browser apps must proxy mainnet relayer requests through their own backend to keep the API key server-side. Node scripts put `auth: { __type: "ApiKeyHeader", value: process.env.RELAYER_API_KEY }` on the chain preset (note the **double-underscore** `__type` — that's the discriminator the relayer-sdk type expects).
