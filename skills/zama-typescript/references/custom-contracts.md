...
- use `useEncrypt` to produce `encryptedValues` and `inputProof` (already ABI-ready hex)
- submit them to the contract call
- read the returned encrypted handles
- use `useDecryptValues` to decrypt the final result

Typical pattern:

1. collect plaintext user input
2. call `useEncrypt`
3. send `encryptedValues` + `inputProof` to the contract
4. read the encrypted handle back from the contract
5. gate `useDecryptValues` behind `useHasPermit`
...
- `encrypt()` returns `{ encryptedValues, inputProof }` already as `0x` hex (`encryptedValues` are `bytes32`, `inputProof` is `bytes`) — pass them straight to contract calls. Do **not** run them through `bytesToHex` / `hexlify`.
- Gate decrypt operations behind a permit check (`hasPermit` / `useHasPermit`) instead of surprising the user with a wallet popup.
